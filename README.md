...
1,558,821 笔、正例 1,813，正例率约 0.116%。数据为公开合成 SAML-D。下表把已发布主线和
后续候选放在同一处展示，避免把“主线未替换”误解为“实验结果未更新”。

| 模型 | 环境 / 角色 | 测试 PR-AUC | 0.1% 告警预算证据 | 当前状态 |
|---|---|---:|---|---|
| **GAT + 时点新颖性** | **Windows 本机冻结候选** | **0.9714** | P/R `0.9814 / 0.8439` | 最新特征候选；待 Linux/跨生成机制复现 |
| CatBoost FE v2 + 历史 GAT v1 | Linux 跨版本融合 | **0.9576** | 找到 `1,536 / 1,813` 个正例 | 权威 sidecar；跨机器结论不稳定 |
| CatBoost FE v2 + GAT FE v2 | Linux 同版本融合 | **0.9554** | 见权威产物 | 权威 sidecar；未替换主线 |
| **GAT v1** | **Linux 已发布主线图排序器** | **0.9483** | P/R `0.9743 / 0.8378` | 当前正式主线 |
| GAT FE v2 | Linux 图 sidecar | 0.9389 | 见权威产物 | 低于 GAT v1，不晋升 |
| CatBoost + GAT v1 | Linux OOF 融合策略 | 0.9175 | P/R `0.9359 / 0.8047` | 融合工程主线；低于单 GAT |
| GraphSAGE | Linux 图对照 | 0.8777 | P/R `0.8736 / 0.7512` | 已被 GAT 替代 |
| CatBoost FE v2 | Linux 表格 sidecar | 0.8754 | 见权威产物 | 相对 v1 CatBoost 提升 |
| CatBoost v1 | Linux 主线表格 | 0.8092 | P/R `0.8576 / 0.7375` | 表格基线 |
| Logistic Regression | Linux 表格对照 | 0.1966 | P/R `0.2809 / 0.2416` | 线性基线 |
| 规则 v2026.2 | Linux 业务基线 | 0.0015 | P/R `0.0019 / 0.0017` | 训练期定阈规则 |
