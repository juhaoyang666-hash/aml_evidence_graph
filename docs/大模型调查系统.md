...
| 缺口 | 当前状态 | 下一步验收 |
|---|---|---|
| 检索评测 | 80 条独立检索 Golden 和四路对照 | 扩充 5 篇 starter 语料并争取独立复核 |
| 工具路由 | 三类受限只读工具，60 案选择/参数评测 | 扩充真实合规裁定与外部 LLM 路径 |
| Human-in-the-loop | checkpoint + approve/edit/reject + SQLite 恢复 + 单进程幂等探针 | 多 worker 共享锁/CAS 与生产集中/WORM 审计 |
| 运行轨迹 | 节点、工具、耗时、状态和审核决定已记录 | 增加统一 trace/run 关联和可视化 |
| Golden 规模 | 生成 34 案、检索 80 条、Agent 60 案，均为项目内裁定 | 需要时另做独立专家复核 |
