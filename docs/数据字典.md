...
特征列不在文档中手工维护一份易过期的长清单；代码注册表与每次 run manifest 是权威来源：

- `src/aml_evidence_graph/features/pit.py`：即时特征、sender/receiver/relationship 历史窗口；
- `src/aml_evidence_graph/features/graph_stats.py`：只读历史图统计；
- `src/aml_evidence_graph/features/fe_v2.py`：FE v2 Typology/金额形态 sidecar；
- 各产物 `_run_manifest.json`：该 run 的实际特征名、配置与输入指纹。

所有历史特征只能读取 `[t-window,t)`；同一时间戳批次先评分再共同入历史。`is_laundering` 与
`laundering_type` 只用于监督和评估切片，禁止进入特征。新增或删除特征时必须更新注册表、
Schema/泄漏测试和 manifest，不再同步维护静态列数。
