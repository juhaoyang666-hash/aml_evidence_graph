...
两者必须使用不同文件：checkpoint 保存可恢复业务状态；audit 文件仅追加值最小化的工具、节点
和人工动作元数据。审计库不保存证据正文、特征值、图边或复核备注正文。该 SQLite 后端用于
本地演示和工程验收，正式部署需换成带访问控制、集中备份和组织保留/销毁策略的审计后端；
运行时不会自动清理记录。这两个文件都位于 `artifacts/`，不得提交 Git。
...
同一键配不同正文会返回 `409`。当前细粒度互斥锁只覆盖单 API 进程，多 worker/多实例部署前
必须换用共享锁或数据库 CAS 并重新做并发验收。
