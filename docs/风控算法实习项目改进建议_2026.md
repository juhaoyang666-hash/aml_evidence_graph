...
1. **大数据工程已有完整数据等价证据，仍不是集群生产作业**：五个代表性 Spark PIT
   特征已在 9,504,852 行上完成全量/增量逐行等价、shuffle、耗时、峰值内存和 0.25 秒
   资源曲线记录；生产集群调度和分布式原生写入仍属外部环境扩展。
2. **Agent 本地工程闭环已形成，仍缺生产共享后端**：60 案 Golden、故障注入、SQLite 恢复、
   独立值最小化审计、400 次单进程审批探针及 160 次两 worker 共享锁审批探针已落地；下一缺口是跨主机事务协调、
   集中/WORM 审计与外部 LLM 路径的 token/cost 覆盖。
3. **RAG 已完成一次有来源扩容，拒答仍需改进**：语料从 5 篇 starter 扩到 11 篇，其中新增
   6 篇 FATF/FinCEN 来源摘要；Golden 从 80 扩到 130 条。新增有来源切片表现较好，但无答案
   误召回升至 `0.304`，且裁定尚未经过独立合规专家复核。
4. **一页式证据生成器已落地，但本机证据不完整**：生成器已校验 run、指纹、score population、
   时间范围、Bootstrap 和主线/sidecar 配对；当前 Windows 缺 v1 三路权威产物，因此按门禁
   保持 `public_ready=false`，必须从 Linux 权威产物生成最终简历页。
5. **FE v2 已闭环但暂不替换主线**：七步链已完成，当前按项目决策保留为 sidecar；在完成
   跨版本复核前，不覆盖 v1 GAT 主线或修改简历数字。
...
| LLM 应用 | 能构建而非只调用聊天 API | Transformer、RAG、Rerank、Agent、工具调用、评测 | **混合检索与受控 Agent 基线已具备；知识库仍小** |
| 金融合规 | 输出可审计、可拒答、有人审 | 证据绑定、事实校验、HITL、权限、回溯、成本/延迟 | **本地持久化、独立审计与幂等已落地；生产共享/WORM 后端仍缺** |
...
**当前落地（2026-07-28）。** FE v2 七步链已经完成并继续作为 sidecar。简历证据生成器已
升级为强门禁：除 metrics/manifest/run purpose 外，还逐个读取冻结 score Parquet，校验测试
行数、正例数、时间范围、概率列及 `[0,1]`，核对 metrics 与 manifest 的 run_id，并记录代码、
输入数据和配置指纹；配置要求的 200 次 Bootstrap 不足时拒绝。生成器会自动配对 v1/FE v2
的 CatBoost、GAT 和融合 PR-AUC，明确输出 improved/regressed/unchanged。当前 Windows 本机
只有三路 FE v2 权威产物，缺 `table_baseline_rules`、`gat`、`fusion_test_cb_gat` 三份 v1
主线产物，因此预览报告保持 `public_ready=false`；不得从文档中的手工数字反向拼装通过。
...
**当前落地（2026-07-28）。** 已建立 130 条检索 Golden（107 可回答、23 拒答），覆盖中文、
同义改写、多 Typology、缺字段、hard negative 和 42 条有来源查询；本地多语言
SentenceTransformer 固定到明确 commit，并完成 BM25/dense/gated hybrid/rerank 对照。语料
由 5 篇 starter 扩至 11 篇，新增 funnel account、money mule、trade-based、virtual asset、
beneficial ownership 和 payment processor 六类 FATF/FinCEN 摘要。

冻结语义门禁的 hybrid 在 Recall@3/MRR/无答案误召回率上为 `0.799/0.752/0.304`，BM25
为 `0.673/0.660/0.565`；新增有来源切片的 hybrid Recall@3/MRR 为 `0.952/0.976`，中文
Recall@3 为 `0.646`。无答案误召回相对旧 80 案的 `0.200` 退化，说明新增词面 hard
negative 有效暴露了拒答问题；本轮未按 v3 调整冻结阈值，也未把 rerank 晋升为默认。裁定仍是
作者标注，因此结果只能作为项目工程证据，不能外推为生产 RAG 质量。
...
初版使用本地 FAISS 或 NumPy 索引即可，不需要为了简历强行部署 Milvus。当前 130 条检索
Golden 已覆盖同义表达、字段缺失、跨 Typology 混合、无相关文档和 hard negative；下一轮
应另建校准/测试划分改善拒答，不能直接在 v3 结果上调 0.35 阈值。
...
**当前落地（更新至 2026-07-31）。** 已实现三个受限只读工具、确定性路由、LangGraph
`interrupt()`、approve/edit/reject、SQLite checkpoint，以及启动、状态查询和审批恢复 API；
跨进程使用同一 `thread_id` 恢复已加入回归测试。新增 60 案合成 Agent Golden（12 路由、
24 工具边界、12 审核、12 恢复），逐案保存 node timeline 与工具审计。确定性无 LLM 基线的
工具选择、参数合法、事实一致和恢复成功率均为 `1.0`，本机 p50/p95 约为 `18.4/111.5 ms`。
这些是项目内合成回归结果，不是生产验收；无外部 LLM 调用，所以 token/cost 覆盖率如实记为
`0.0`。checkpoint 外的独立 SQLite 追加式审计也已接入受控 API：事件 ID 幂等，重开后可按
`thread_id` 查询，并且 Schema 不提供证据正文、特征值、图边或复核备注正文列。详见
[Agent调查评估.md](Agent调查评估.md)。单进程 HTTP 幂等探针也已完成：50 个案件、每案 8 个
并发同键审批，共 400 个 review 请求，错误率 `0%`；350 个请求为回放，审计库只产生 50 条
review 和 50 条 finalize。随后增加共享 SQLite EvidenceStore、checkpoint、审计库和线程租约，
两个真实 Uvicorn worker 下 40 次证据读取无 404；160 个同键审批请求只有 20 次实际执行，
140 次为回放，review/finalize 审计各 20 条。剩余重点是跨主机事务协调、外部 LLM 路径复跑，
以及把本地审计原型替换为生产集中/WORM 适配器与组织保留策略。

**问题。** 当前状态图仍是确定性串行受控链，不应包装为自主规划 Agent；已具备持久化、
暂停/恢复、审批、本地独立审计、单进程幂等回放和本机两 worker 对同一 `thread_id` 的共享
互斥，但没有跨主机一致性或生产集中/WORM 审计后端。携程、Shopee、国金岗位尤其看重
Function Calling、流程拆解、HITL 与可回溯，因此后续应补跨主机故障边界和生产审计适配，
而不是增加未经评测的自主性。
...
**当前落地（2026-07-28）。** 已实现异步压测客户端和聚合报告生成器，Mock 健康检查、调查
草稿和受控 Agent 完整周期已有本机实测。Agent 的 50 个启动→查询→审批恢复周期在并发 5 下
错误率为 `0%`，吞吐 `49.73 cycle/s`，p95 `147.16 ms`。v1 CatBoost 冻结评分对完整 PIT
日期分区串行请求 10 次，错误率 `0%`、p95 `633.43 ms`。聚合报告现为完整，但这里的评分
吞吐是分区/秒且不含 GAT/融合，不能写成单笔在线延迟；所有结果均声明不是生产 SLA。
...
**当前落地（2026-07-31）。** 已通过清华镜像安装 PySpark 3.5.9 与 OpenJDK 17.0.14，
实现 sender 1d/7d/30d、receiver 7d、relationship 7d 五个历史计数特征。窗口以 epoch 秒
排序并严格 `rangeBetween(-window, -1)`，同秒交易不会互相泄漏。完整 9,504,852 行全量重放
耗时 71.29 秒、峰值进程树内存 12,112.4 MB；最后一天增量扫描 885,238 行并输出
8,400 行，耗时 16.39 秒。五个特征全量/增量 Match rate 均为 `1.0`，最大绝对误差为 0。
物理计划记录 4 个 Exchange；对齐仅使用 `transaction_id` 显式等值连接，禁止普通
笛卡尔积。Windows 因缺 Hadoop native writer 使用 Arrow driver 写出结果，不能表述为
生产集群写入性能；Linux/集群走原生 Spark writer。详见
[批量特征重放.md](批量特征重放.md)。
...
**当前落地（2026-07-31）。** 已通过清华镜像安装 `mlflow 3.14.0`，保留 manifest 为权威
来源，将 FE v2 的 table、GAT、融合拟合和融合测试 4 个完成产物同步到本地 SQLite tracking。
同步只写聚合 metrics、manifest 与来源标签，不写交易行或模型权重；同一 source run 重复同步
返回同一 FINISHED run。真实接入时发现告警预算的 `%` 不符合 MLflow metric name 约束，最初
3 个 run 被记录为 FAILED；修复名称规范化后成功同步，报告保留失败/成功状态作为工程证据。
candidate gate 会拒绝任何名称含 `test` 的指标。现已补齐 Windows 同时间切分、同逐行人群的
CatBoost、GAT 和固定 CatBoost FE v2 融合组件对照；表格 v1 指标由保留的完整分数重算，
来源指纹可追溯。三组 gate 仅用 validation PR-AUC，结论为 CatBoost FE v2 通过，GAT FE v2
与同版本融合不通过。详见[特征工程V2实验.md](特征工程V2实验.md)。
...
| P0 | 混合 RAG + 检索 Golden | 3—5 天 | 拒答校准集/独立复核 | Recall@k/MRR/nDCG、hard negative |
...
- P0-2：实现 BM25、TF-IDF 与固定版本多语言 SentenceTransformer、gated RRF、可选 rerank
  和独立检索指标；语料已扩到 11 篇、Golden 已扩到 130 条并输出来源/标签切片与逐案
  Bad Case。冻结门禁在 v3 的拒答退化被如实保留。
- P0-3：实现三个结构化只读工具、最多四次确定性路由、LangGraph checkpoint、强制
  approve/edit/reject 中断恢复，以及与 checkpoint 分离、事件幂等且不记录正文/特征值的
  SQLite 追加式审计原型。
- P0-1/P0-4：实现完整产物驱动的简历证据生成器与有界并发 HTTP 压测；证据门禁校验
  score population、时间范围、概率列、run_id、指纹、Bootstrap 和主线/sidecar 配对，不会
  从运行中或缺失产物提取简历数字；本机性能也不表述为生产 SLA。
- P1-1：五个代表性 Spark PIT 窗口特征已完成全量/增量逐行等价和资源验收；窗口严格
  `[t-window, t)`，对齐使用显式等值连接，代码不包含普通笛卡尔积。
- P1-2：MLflow 本地 SQLite tracking 已验收 4 个 FE v2 完成产物；适配层只同步聚合指标并
  保留 manifest 为权威记录，失败 run 可识别、重复同步幂等、candidate gate 拒绝测试指标。

环境采用 extras 分组，锁文件已更新；当前 GPU 版 PyTorch 与 SQLite checkpoint 可用，
SentenceTransformer 已安装并固定模型 commit；MLflow 已完成本地验收；PySpark 3.5.9 与
OpenJDK 17.0.14 已安装并完成本地等价链。详细命令和未完成项见
[`项目改进进度.md`](项目改进进度.md) 与 [`环境配置.md`](环境配置.md)。
...
- 一份唯一的 `简历证据.md`，包含数据、切分、模型、指标、run_id 与硬件。
