...
3. **RAG 语料规模仍小**：已有 80 条检索 Golden 和 BM25/语义/混合/rerank 对照，但
   Typology starter 库只有 5 篇，且裁定尚未经过独立合规专家复核。
...
**当前落地（2026-07-28）。** 已建立 80 条检索 Golden（65 可回答、15 拒答），覆盖中文、
同义改写、多 Typology 与 hard negative；本地多语言 SentenceTransformer 固定到明确 commit，
并完成 BM25/dense/gated hybrid/rerank 对照。gated hybrid 的 Recall@3/MRR/无答案误召回率为
`0.785/0.731/0.200`，BM25 为 `0.615/0.567/0.467`。语料仍只有 5 篇，裁定仍是作者标注，
因此结果只能作为项目工程证据，不能外推为生产 RAG 质量。
...
- P0-2：实现 BM25、TF-IDF 与固定版本多语言 SentenceTransformer、gated RRF、可选 rerank
  和独立检索指标；检索 Golden 已扩到 80 条并输出标签切片与逐案 Bad Case。
