...
2. **Agent 已有正式回归基线，仍缺生产审计**：60 案受控 Agent Golden、故障注入、SQLite
   恢复、逐节点时间线和 Bad Case 输出已落地；下一缺口是重复审批幂等压测、独立审计存储与
   外部 LLM 路径的 token/cost 覆盖。
...
| 金融合规 | 输出可审计、可拒答、有人审 | 证据绑定、事实校验、HITL、权限、回溯、成本/延迟 | **持久化 HITL 已落地；生产审计存储仍缺** |
...
[Agent调查评估.md](Agent调查评估.md)。剩余重点是并发恢复幂等压测、外部 LLM 路径复跑，
以及把本地审计原型替换为生产集中/WORM 适配器与组织保留策略。

**问题。** 当前状态图仍是确定性串行受控链，不应包装为自主规划 Agent；已具备持久化、
暂停/恢复、审批和本地独立审计，但尚未证明多个 API worker 对同一 `thread_id` 的并发互斥，
也没有生产集中/WORM 审计后端。携程、Shopee、国金岗位尤其看重 Function Calling、流程拆解、
HITL 与可回溯，因此下一步应补并发正确性与生产边界，而不是增加未经评测的自主性。
...
SentenceTransformer 已安装并固定模型 commit；MLflow 已完成本地验收，PySpark 仍按需安装。详细命令和未完成项见
[`P0框架状态.md`](P0框架状态.md)。
