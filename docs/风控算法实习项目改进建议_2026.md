...
1. **大数据工程已有等价证据，仍不是集群生产作业**：五个代表性 Spark PIT 特征已在
   355,998 行 smoke 上完成全量/增量逐行等价、shuffle、耗时和峰值内存记录；尚缺真实集群
   调度、分布式写入和千万级资源曲线。
...
**当前落地（2026-07-28）。** 已通过清华镜像安装 PySpark 3.5.9 与 OpenJDK 17.0.14，
实现 sender 1d/7d/30d、receiver 7d、relationship 7d 五个历史计数特征。窗口以 epoch 秒
排序并严格 `rangeBetween(-window, -1)`，同秒交易不会互相泄漏。355,998 行 smoke 全量重放
与官方 PIT 逐行 Match rate 均为 `1.0`，耗时 17.06 秒、峰值进程树内存 1,181.5 MB；目标日
增量重放扫描 56,694 行、输出 28,155 行，与全量目标日也全部 `1.0`。物理计划记录 4 个
Exchange、shuffle partitions=8；对齐仅使用 `transaction_id` 显式等值连接，禁止普通
笛卡尔积。Windows 因缺 Hadoop native writer 使用 Arrow driver 写出 smoke 结果，不能表述为
