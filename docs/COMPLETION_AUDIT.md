...
| 私有 CSV 到 token 化 Parquet、质量清单与运行清单 | 已实现，待真实数据执行 | `data/configuration.py` 锁定切分与缺失率门槛；`ingestion/profile.py`、`ingestion/prepare.py`、`tracking/run.py`。 |
...
- `python -m pytest`：68 passed（仅含合成/Mock 测试，不读取私有流水）。
...
1. 当前会话没有 `AML_TOKENIZATION_SECRET`，因此不能安全地执行真实 CSV 转换、全量训练、冻结融合与一次性测试评估；不得填写真实 PR-AUC、召回、延迟或 run_id。
2. 100 个经双人标注的脱敏 Golden cases、调查容量预算、模型晋升阈值需要业务和合规方批准；Mock case 不能替代该验收。
3. 当前主机没有 Docker CLI，Dockerfile/compose 已通过静态 Mock 隔离检查，但尚未完成本机镜像构建和启动验证。
