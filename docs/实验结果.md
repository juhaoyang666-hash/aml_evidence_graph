...
**Primary sources**: `artifacts/table_baseline_fe_v2`,
`artifacts/gat_cold_start_v3_novelty_only_60d_seed20260723_frozen_test`, and
`artifacts/fusion_test_catboost_fe_v2_gat_v1_local_replay`; rules/LR/GraphSAGE retain
...
| **CatBoost FE v2** | **primary table** | **0.8754** | 0.9984 | **0.8999** | **0.7739** | 0.2194 | 0.9432 | `20260727T101815Z-14ce85d76a` |
| **GAT + event-time novelty** | **primary graph** | **0.9714** | 0.9998 | **0.9814** | **0.8439** | 0.2312 | 0.9939 | checkpoint `8c7bb6cf63ec` |
| **fusion (CatBoost FE v2 + local GAT v1 replay)** | **OOF fusion (2-way)** | **0.9505** | 0.9998 | **0.9737** | **0.8373** | 0.2298 | 0.9879 | `20260728T092504Z-90fdda2285` |

Notes:

- Main-line reporting: **CatBoost FE v2**, **GAT + event-time novelty**, and the precisely
  versioned **`CatBoost FE v2 + local GAT v1 replay` OOF fusion**. GraphSAGE is reported
...
> CatBoost FE v2 已晋升为当前表格主线；GAT FE v2 未晋升。以下数字保留实验演进与
...
- CatBoost FE v2 当前替代 v1 CatBoost 作为主线表格结果；v1 仍作为历史基准保留。
...
> FE v2 reaches test PR-AUC 0.875; at a 0.1% alert budget, Precision 0.900 / Recall
> 0.774. A history-only GAT edge classifier with event-time novelty features reaches
> test PR-AUC 0.971 (0.1% budget P 0.981 / R 0.844). Leakage-safe expanding-time OOF
> fusion of CatBoost FE v2 and a local GAT v1 replay reaches test PR-AUC 0.950
> (0.1% budget P 0.974 / R 0.837). Versus train-period quantile rules, CatBoost FE v2
