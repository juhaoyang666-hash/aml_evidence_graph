...
- Main-line reporting: **CatBoost FE v2**, **GAT + event-time novelty**, and the precisely
  versioned **`CatBoost FE v2 + local GAT v1 replay` OOF fusion**. GraphSAGE is retained
  for architecture comparability, not as the recommended scorer.
...
- 复现入口：`scripts/run_fe_v2_pipeline_resumable.py`（PIT sidecar + table）；
  图/OOF/融合链见 `scripts/run_fe_v2_gat_oof_fusion.ps1`。
