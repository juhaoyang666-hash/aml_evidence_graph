...
- Linux 权威 sidecar（简历门禁）：GAT `0.93887`、同版本融合 `0.95540`；
  跨版本融合见 [GATv1与CatBoostFEv2融合实验.md](GATv1与CatBoostFEv2融合实验.md)。
...
See [CatBoost差距诊断.md](CatBoost差距诊断.md). At 0.1% budget, GAT recovers
...
[CatBoost差距诊断.md](CatBoost差距诊断.md).
...
see [实验附录.md](实验附录.md). **Does not replace** single-rel RGCN in
...
|---|---|---|
| Monthly ops + threshold recalibration | [漂移监控.md](漂移监控.md) | `artifacts/drift_monitoring` |
| High-score subgraph Louvain / CC | [实验附录.md](实验附录.md) | `artifacts/community_baseline` |
| Isolation Forest / AE | [实验附录.md](实验附录.md) | `artifacts/unsupervised_baseline` |
| Multi-rel RGCN (R=4) | [实验附录.md](实验附录.md) | `artifacts/rgcn_rel` |
| Sequence GRU | [实验附录.md](实验附录.md) | `artifacts/sequence_baseline` |
| GAT→CatBoost distill | [实验附录.md](实验附录.md) | `artifacts/table_baseline_gat_distill` |
| Nonlinear OOF fusion heads | [实验附录.md](实验附录.md) | `artifacts/nonlinear_fusion` |
| DuckDB/Polars feature replay | [批量特征重放.md](批量特征重放.md) | `artifacts/batch_feature_replay` |
...
- **A3 relation MLP**: with_rel PR-AUC 0.0363 vs no_rel 0.0379 (helps=False). See [实验附录.md](实验附录.md).
- **A3 multi-rel RGCN (R=4)**: test PR-AUC **0.8873** (val 0.8412) · run `20260726T135457Z-045b0b06ab` · below single-rel RGCN 0.903 / GAT 0.948. [实验附录.md](实验附录.md).
- **B1 sequence GRU**: sampled test PR-AUC 0.0103. [实验附录.md](实验附录.md).
- **B2 GAT distill CatBoost**: test PR-AUC 0.9656 (beats_catboost=True). [实验附录.md](实验附录.md).
- **B3 nonlinear fusion**: logistic 0.9177 > best nonlinear 0.9073. [实验附录.md](实验附录.md).
- **B4 batch replay**: DuckDB/Polars match rates 1.000/1.000. [批量特征重放.md](批量特征重放.md).
- **B5 LLM probes**: Golden expanded to 34 cases; template hallucination intercept 1.0. [大模型调查系统.md](大模型调查系统.md).
