...
| GAT/RGCN/PNA 全量对比 | 成熟版目标 | **已完成**（附加对比；主线仍为 CatBoost / GraphSAGE / 双路融合），见 §4 / RESULTS |
...
4. 引用示例（可直接用于简历）：
   ...
   > 时间外测试期取得 PR-AUC 0.806；0.1% 告警预算下 Precision 0.845 / Recall 0.727。
   > GraphSAGE（历史图、无标签推理）测试期 PR-AUC 约 0.878。相对规则基线报告同召回告警削减。
