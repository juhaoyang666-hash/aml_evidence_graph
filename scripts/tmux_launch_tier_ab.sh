...
launch aml_a3_rel "$PY scripts/run_relation_ablation.py 2>&1 | tee logs/relation_ablation.log"

# B2 GAT distill CatBoost — hours
launch aml_b2_distill "$PY scripts/run_gat_distill_catboost.py 2>&1 | tee logs/gat_distill.log"

# B1 sequence GRU — hours (loads full prepared splits)
launch aml_b1_seq "$PY scripts/run_sequence_baseline.py 2>&1 | tee logs/sequence_baseline.log"

# B4 batch replay — minutes–1h
launch aml_b4_batch "$PY scripts/run_batch_feature_replay.py 2>&1 | tee logs/batch_feature_replay.log"
